...
func checkConfig(c *cli.Context) error {
	detail_aux := true
	return check.Config(c.Args().First(),detail_aux)
}
...
/*
Public/Private/Ephemeral key creation, as described in report
*/
...
func gen_keys(c *cli.Context) error{
	if len(c.Args().First()) == 0 {
		return errors.New("Please input a configuration file")
	}else{
